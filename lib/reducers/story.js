...
function branchesByRepoId(state = new Map(), action = {}) {
  switch (action.type) {
  case PATCH_STORY:
    // FIXME: how to check typeof variable === 'Map' ?
    if (action.payload.branchesByRepoId) {
      return new Map([...state, ...action.payload.branchesByRepoId]);
    }
    return state;
  default:
    return state;
...
export default function(state = new Map(), action = {}) {
  return new Map([
    ...
    ['branchesByRepoId', branchesByRepoId(state.get('branchesByRepoId'), action)],
