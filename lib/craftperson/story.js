...
function mixWatchedAndNotUserRepos(...repos) {// eslint-disable-line no-unused-vars
  return [];
}
...
function fetchWatchedReposPromise({octo, since} = {}) {// eslint-disable-line no-unused-vars
...
function fetchReposIssuesPromise({repos, since} = {}) {// eslint-disable-line no-unused-vars
  return Promise.resolve([]);
}
...
export function fetchCraftedStories(configs = new Map()) {
...
function mergePullRequestPromiseByIdentifiers({octo, identifiers = []} = {}) {
  return Promise.all(
    identifiers.map((identifier) => {
      return octo.repos(identifier.owner, identifier.repo)
...
export function fetchCraftedStoriesByIssue({configs = new Map(), identifiers = []} = {}) {
  return new Promise((resolve1) => {
    const token = decryptData(configs.get('token'));
    const octo = new Octokat({
      token: token,
...
export function toggleIssuesState({configs = new Map(), identifiers = []} = {}) {
  return new Promise((resolve1) => {
    const token = decryptData(configs.get('token'));
    const octo = new Octokat({
      token: token,
...
export function deletePullRequestsBranch({configs = new Map(), identifiers = []} = {}) {
  return new Promise((resolve1) => {
...
export function mergePullRequests({configs = new Map(), identifiers = []} = {}) {
  return new Promise((resolve1) => {
    const token = decryptData(configs.get('token'));
    const octo = new Octokat({
      token: token,
    ...
    return mergePullRequestPromiseByIdentifiers({
      octo: octo,
      identifiers: identifiers,
    }).then(() => {
      return fetchCraftedStoriesByIssue({
