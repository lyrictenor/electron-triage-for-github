...
export function buildIdentifiersForBranchesFromRepos({ repos = new Set(), reposById = new Map() }) {
  const identifiers = [];
...
export function normalizeBranches(branchesAndParams) {
  const result = {
    branchesByRepoId: new Map(),
    branches: new Set(),
  };
  for (const branchesAndRepo of branchesAndParams) {
    const [branch, repoId] = branchesAndRepo;
    result.branches.add(repoId);
    const repoBranches = new Map();
    for (const oneBranch of branch) {
      repoBranches.set(oneBranch.name, oneBranch);
    }
    result.branchesByRepoId.set(repoId, repoBranches);
  }
  return result;
}

export function fetchBranchesByIdentifiers({configs = new Map(), identifiers = []} = {}) {
  const octo = buildOcto(configs);
  return Promise.all(
    identifiers.map((identifier) => {
      return Promise.all([
        ...
        Promise.resolve(identifier.repoId),
      ]);
    })
  ).then((value) => {
    return normalizeBranches(value);
  });
}
...
export function toggleIssuesStateByIdentifiers({configs = new Map(), identifiers = []} = {}) {
  const octo = buildOcto(configs);
  return Promise.all(
    identifiers.map((identifier) => {
...
export function deletePullRequestsBranchByIdentifiers({configs = new Map(), identifiers = []} = {}) {
  return fetchIssuesByIdentifiers({
    ...
    const [ pullsStory, branchesStory ] = value;
    const { pullsByKey, pulls } = pullsStory;
    const { branchesByRepoId, branches } = branchesStory;
    const removing = [];
    for (const pullKey of pulls) {
      if (pullsByKey.has(pullKey)) {
        const pull = pullsByKey.get(pullKey);
        if (pull && pull.head && pull.head.repo) {
          const repoId = pull.head.repo.id;
          const branchName = pull.head.ref;
          if (branches.has(repoId) && branchesByRepoId.has(repoId)) {
            const repoBranches = branchesByRepoId.get(repoId);
            if (repoBranches.has(branchName)) {
              removing.push(pull.head.repo.git.refs.heads(branchName).remove());
            }
          }
        }
      }
...
export function mergePullRequestsByIdentifiers({configs = new Map(), identifiers = []} = {}) {
  const octo = buildOcto(configs);
  return Promise.all(
    identifiers.map((identifier) => {
      return octo.repos(identifier.owner, identifier.repo)
...
export function fetchReposIssuesByIdentifiers({configs = new Map(), identifiers = []} = {}) {
  const octo = buildOcto(configs);
  const now = new Date();
  const since = new Date(now - 5 * 24 * 60 * 60 * 1000);
  return Promise.all(
    ...
    return value.map((story) => {
      const [issuesArray, repo] = story;
      const issues = issuesArray.reduce((prev, current) => {
        return prev.concat(current);
      }, []);
...
export function fetchReposAllIssuesByIdentifiers({configs = new Map(), identifiers = []} = {}) {
  const octo = buildOcto(configs);
  return Promise.all(
    identifiers.map((identifier) => {
      return Promise.all([
